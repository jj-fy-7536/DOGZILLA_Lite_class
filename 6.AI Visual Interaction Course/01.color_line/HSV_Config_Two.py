...
BLACK_MAX_VALUE = 80
BLACK_MAX_CHROMA = 35
DEBUG_MARK_RGB = (255, 255, 0)
DEBUG_TEXT_RGB = (255, 0, 255)
...
class update_hsv:
    ...
    def draw_contours(self, hsv_name, contours):
        ...
        # 绘制中?
        cv.circle(self.image, (x, y), 5, DEBUG_MARK_RGB, -1)
        # 计算最小矩形区域
        ...
        # 绘制最小矩形
        cv.drawContours(self.image, [box], 0, DEBUG_MARK_RGB, 2)
        cv.putText(self.image, hsv_name, (int(x - 15), int(y - 15)),
                   cv.FONT_HERSHEY_SIMPLEX, 1, DEBUG_TEXT_RGB, 2)
