...
- 这个常驻监听器同时处理任务触发、停止、继续和 Spark Lite 简单问答，避免同时开两个 `arecord` 抢麦克风。
...
非任务问句/请求: 复用 `voice_interaction.py` 的 Spark Lite 问答过滤和 HTTP 调用
```
...
密钥文件不要放进本目录。机器狗上仍建议把讯飞、在线语音合成和 Spark 密钥放在:
...
- 语音识别需要 `XFYUN_APPID`、`XFYUN_API_KEY`、`XFYUN_API_SECRET`；在线合成可单独配置 `XFYUN_TTS_APPID`、`XFYUN_TTS_API_KEY`、`XFYUN_TTS_API_SECRET`；问答还需要 `SPARK_API_PASSWORD`。
